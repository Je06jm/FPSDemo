...
func _ready():
	...
	_is_joy = true
	_settings_set_sensitivity(config["joy_sensitivity"])
	_is_joy = false
	_settings_set_sensitivity(config["mouse_sensitivity"])
...
func _toggle_menu(joy : bool):
	...
	if visible:
		...
		if joy:
			$Settings/Sensitivity.value = GameState.joy_sensitivity
			$Settings/Sensitivity.step = 7
		
		else:
			$Settings/Sensitivity.value = GameState.mouse_sensitivity
			$Settings/Sensitivity.step = 1
...
func _settings_set_sensitivity(value : int):
	if _is_joy:
...
func _Settings_on_Sensitivity_value_changed(value):
	_settings_set_sensitivity(value)
	save_file = true
